// Shared seeding helper for LATAM city content batches.
// Usage: const { seed } = require('./_latam-seed-helper');
//        seed({ SPOTS, EXPERIENCES });
const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

// Map experience category -> an existing fallback image (only -1/-2 exist per folder).
const EXP_IMG = {
...
// Map spot category -> existing fallback image.
const SPOT_IMG = {
...
// Railway's pooled connection drops intermittently (P1017). Retry transient errors.
...
async function seed({ SPOTS = {}, EXPERIENCES = {} }) {
  // Resolve city slugs -> ids
  const cities = await prisma.city.findMany({
    where: { region: 'LATIN_AMERICA' },
    select: { id: true, slug: true },
  });
  const cityMap = {};
  cities.forEach((c) => (cityMap[c.slug] = c.id));

  const organizer = await prisma.experienceOrganizer.findFirst();
    ...
    for (const e of exps) {
      ...
      await withRetry(() => prisma.experience.create({
        data: {
          ...
          category: e.category,
          duration: e.duration,
          groupSizeMin: e.groupSizeMin ?? 1,
          groupSizeMax: e.groupSizeMax ?? 12,
